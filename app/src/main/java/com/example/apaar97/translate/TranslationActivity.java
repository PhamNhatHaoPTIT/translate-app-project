...
public class TranslationActivity extends AppCompatActivity implements TextToSpeech.OnInitListener {
    ...
    private class TranslateText extends AsyncTask<String,Void,String> {
        ...
        private void insertWordToDB(String result) {
            WordDB wordDB = new WordDB(TranslationActivity.this);
            Cursor cursor = wordDB.getAllWord();
            boolean isInsert = true;
            if(cursor != null) {
                while (cursor.moveToNext()) {
                    if(cursor.getString(3).equals(mTextInput.getText().toString())) {
                        isInsert = false;
                        break;
                    }
                }
            }
            Word word = new Word(mLanguageCodeFrom, mLanguageCodeTo, mTextInput.getText().toString(), result);
            notificationDialog(word.getCodeFrom() + " - " + word.getWordFrom(), word.getCodeTo() + " - " + word.getWordTo());
            if(isInsert) {
                wordDB.add(word);
            }
        }

        private void notificationDialog(String from, String to) {
            NotificationManager notificationManager = (NotificationManager) getSystemService(TranslationActivity.this.NOTIFICATION_SERVICE);
            String NOTIFICATION_CHANNEL_ID = "haopn";
            ...
            NotificationCompat.Builder notificationBuilder = new NotificationCompat.Builder(TranslationActivity.this, NOTIFICATION_CHANNEL_ID);
